    ...
    if ( pixel_on_triangle ){
      ...
      if ( draw_pixel ){
        ...
        vec3 fragNormal = (mWorld * vec4(vertNormal, 0.0)).xyz;
            ...
            ivec4 texel0 = D_texture2D(sampler0, fragTexCoord);
            ivec4 texel1 = D_texture2D(sampler1, fragTexCoord);
            ...
            gl_FragColor = vec4(col_transfer(division(division(texel0.rgb * lightIntensity, 1000)) *texel1.rgb, 1000 ) , 1.0);
